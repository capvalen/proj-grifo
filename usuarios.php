...
<body>
					...
					<div class="container">
						...
						<div id="navbar" class="navbar-collapse collapse ">
							...
							<ul class="nav navbar-nav pull-right">
								...
								<li id="liDatosPersonales"><a href="#!">
										<p><strong>Usuario: </strong> <span class="mayuscula" id="menuNombreUsuario"><?php echo $_SESSION["Atiende"]; ?></span></p><small class="text-muted text-center" id="menuFecha"><span id="fechaServer"></span> <span id="horaServer"></span> </small>
									</a></li>
	...
	<script>
		...
		$('#btnGuardarAddUser').click(function() {
			...
			if ($('#txtModalApellidUser').val() == '') {
				$('.modal-addUserBD .labelError').removeClass('hidden').find('.mensaje').text('Debe ingresar los apellidos.');
			} else if ($('#txtModalNombUser').val() == '') {
				$('.modal-addUserBD .labelError').removeClass('hidden').find('.mensaje').text('Debe ingresar los nombres.');
			} else if ($('#txtModalDniUser').val() == '') {
				$('.modal-addUserBD .labelError').removeClass('hidden').find('.mensaje').text('Debe ingresar un Dni.');
			} else if ($('#txtModalNickUser').val() == '') {
				$('.modal-addUserBD .labelError').removeClass('hidden').find('.mensaje').text('Debe ingresar un nick para iniciar sesión.');
			} else if ($('#txtModalPassUser').val() == '') {
				$('.modal-addUserBD .labelError').removeClass('hidden').find('.mensaje').text('Debe ingresar una contraseña.');
			} else if (idNivel === null || idNivel === undefined) {
				$('.modal-addUserBD .labelError').removeClass('hidden').find('.mensaje').text('Debe selecionar un nivel.');
			} else {
				...
				$.ajax({
					...
					type: 'POST',
					data: {
						nombres: $('#txtModalNombUser').val(),
						apellidos: $('#txtModalApellidUser').val(),
						nick: $('#txtModalNickUser').val(),
						pass: $('#txtModalPassUser').val(),
						poder: idNivel
					}
				}).done(function(resp) {
					console.log(resp)
		...
		$('#btnGuardarUpdUser').click(function() {
			if ($('#txtModalUpdPassUser').val() != '') {
				$.ajax({
					url: 'php/actualizarContrasenaUser.php',
					type: 'POST',
					data: {
						idUser: $('#idUserModf').text(),
						pass: $('#txtModalUpdPassUser').val()
					}
				}).done(function(resp) {
					console.log(resp)
				});
			...
			$.ajax({
				url: 'php/actualizarDatosUser.php',
				type: 'POST',
				data: {
